...
# Build date is commit provenance, not a per-invocation timestamp. A wall-clock
# value changes the linker action on every hotfix build and defeats GOCACHE.
...
.PHONY: build-production check-release-dist-ignore

## build: compile gc binary with version metadata
build:
	@$(MAKE) build-production OUTPUT=$(BUILD_DIR)/$(BINARY)
ifeq ($(shell uname),Darwin)
	@scripts/sign-darwin-local.sh $(BUILD_DIR)/$(BINARY)
endif

## build-production: compile gc through the measured, persistent-cache build entrypoint
build-production:
	@scripts/build-production "$(OUTPUT)" "$(LDFLAGS)"
