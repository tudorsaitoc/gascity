...
//
// Core is always included. It ships the role prompts referenced by implicit
// agents and the overlay/per-provider hook files, so its content must reach
// PackOverlayDirs even when the user has never run `gc init` (and therefore
// has no implicit-import.toml written to $GC_HOME). When the beads provider is
// "bd" (the default), include bd and let its own pack includes pull in dolt
